...
function draw() {
  ...
  // Pintar el río de celeste
  ctx.fillStyle = "#00ccff";
  ctx.fillRect(0, 5 * tileSize, canvas.width, 3 * tileSize);
...
function spawnObstacles() {
  ...
  for (let i = 0; i < 5; i++) {
    cars.push({
      ...
      y: 13 - i,
      speed: i % 2 === 0 ? -0.1 - level * 0.01 : 0.1 + level * 0.01
    });
  }
  ...
  for (let i = 0; i < 4; i++) {
    logs.push({
      x: Math.floor(Math.random() * gridWidth),
      y: 6 - i,
      speed: i % 2 === 0 ? 0.05 + level * 0.005 : -0.05 - level * 0.005
    });
  }
